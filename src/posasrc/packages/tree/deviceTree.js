...
define([], function () {
    //zTree
    var zTree = require('../ztree/3.5.24/js/jquery.ztree.all.min');
    var zTreeExHide = require('../ztree/3.5.24/js/jquery.ztree.exhide.min');
    var zTree = require('./data.json');
    // var Urls = {
    //     device_tree: window.ctx + "/system/device/tree/expand", //级联展开树
    ...
    var service = {
        ...
        initTree: function (setting) {
            ...
            setting['reqCallback']().then(function (data) {
                ...
                var zTreeSetting = {
                    async: {
                        enable: setting.isAsync,
                        type:"get",
                        url: function(treeId,treeNode){
                            var url = window.ctx + "/system/device/tree/expand";
                            if(setting.isBaselib){
                                url = window.ctx + "/system/device/tree/expand_append";
                            }
                            if(treeNode){
                                 url=url+"?uuid="+treeNode.id+"&parent="+treeNode.ext.parentId;
                            }
                ...
                var ztree = $.fn.zTree.init(zTreeNode, zTreeSetting, dataList);
